...
begin
  begin
    perform public.mutate_budget_transaction(
      ...
      'update',
      '{"title":"stale lock fixture","amount":8,"type":"expense","category":"food","created_by_member_id":"80000000-0000-0000-0000-000000000001","splits":[]}'::jsonb
    );
    succeeded := true;
  exception when others then
    ...
    if error_code <> '55P03' or elapsed_ms > 1500 then
      raise exception 'transaction lock wait contract failed';
    end if;
  end;
  if succeeded then
    raise exception 'locked stale transaction unexpectedly succeeded';
  end if;
end
$$;
rollback;
